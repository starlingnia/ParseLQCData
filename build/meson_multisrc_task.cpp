...
std::filesystem::path find_multi_meson_dir(const std::string& beta_str) {
...
int measure_channel_multi(std::string beta_str, std::string ch, size_t binsize) {
    ...
    const auto input_dir = find_multi_meson_dir(beta_str);
    const std::filesystem::path output_file = "output/pickdata/b4." + beta_str + "/save_" + ch + "_cpp.csv";

    const auto t0 = std::chrono::steady_clock::now();
    const auto res = lqcd::meson::run_meson_pipeline(input_dir, it->second, binsize, 48, 0, false);
    const auto t1 = std::chrono::steady_clock::now();

    const double elapsed_s = std::chrono::duration<double>(t1 - t0).count();
    std::println("Meson Multi 测量完成 (Beta: 4.{}, 信道: {}, 耗时: {:.3f}s, 构型数: {}, Bins: {})",
...
int run_meson_multi_standalone_task(std::span<const std::string_view> args) {
    ...
    return measure_channel_multi(beta_str, channel, binsize);
}
...
int run_meson_multi_all_task(std::span<const std::string_view> args) {
    ...
    for (const auto& [ch, _] : channel_defs) {
        ...
        measure_channel_multi(beta_str, ch, binsize);
    }
    return 0;
}

#ifndef COMPILING_ALL_TASKS
REGISTER_TASK("meson_multi", run_meson_multi_standalone_task, "Run multi-source meson measurement: [beta] [channel] [binsize]");
