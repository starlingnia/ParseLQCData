...
/**
 * @brief 介子关联函数高并发分析流水线 C ABI 导出接口
 */
...
/**
 * @brief 专职多源介子流水线 C ABI
 */
int run_meson_multisrc_pipeline_c_api(
    const char* input_dir,
    const char* channel_types_csv,
    const char* channel_dirs_csv,
    int binsize,
    ...
    int* out_n_bins,
    int* out_n_raw_cfgs) {
    return run_meson_pipeline_c_api(
        input_dir, channel_types_csv, channel_dirs_csv, binsize,
        num_lines, thread_count, 0, out_means, out_errors,
        out_folded_jk, out_n_bins, out_n_raw_cfgs
    );
}

/**
 * @brief 专职单源介子流水线 C ABI
 */
int run_meson_singlesrc_pipeline_c_api(
    const char* input_dir,
    const char* channel_types_csv,
    const char* channel_dirs_csv,
    int binsize,
    ...
    return run_meson_pipeline_c_api(
        input_dir, channel_types_csv, channel_dirs_csv, binsize,
        num_lines, thread_count, 1, out_means, out_errors,
        out_folded_jk, out_n_bins, out_n_raw_cfgs
    );
}

/**
 * @brief 释放由 C++ 分配给 Python 的缓冲区内存
 */
