...
\midrule
\texttt{L48T18 (流1)} & $48^3 \times 18$ & 136.28 & 0.002/0.04 & 98  & $1.63(8) \times 10^{-3}$  & $1.42(7) \times 10^{-2}$  & $8.30(44) \times 10^{-4}$ \\
\texttt{L48T18 (流2)} & $48^3 \times 18$ & 136.28 & 0.002/0.04 & 76  & $9.02(514) \times 10^{-5}$ & $2.31(231) \times 10^{-4}$ & $7.95(466) \times 10^{-5}$ \\
\texttt{L40T16}       & $40^3 \times 16$ & 153.31 & 0.002/0.04 & 879 & $4.14(85) \times 10^{-5}$  & $1.59(56) \times 10^{-4}$  & $3.33(74) \times 10^{-5}$ \\
\texttt{L32T14}       & $32^3 \times 14$ & 175.21 & 0.002/0.04 & 291 & $3.47(347) \times 10^{-6}$ & $6.00(600) \times 10^{-5}$ & $-1.15(115) \times 10^{-8}$ \\
\texttt{L32T12}       & $32^3 \times 12$ & 204.41 & 0.002/0.04 & 108 & $6.86(43) \times 10^{-4}$  & $1.22(7) \times 10^{-2}$  & $-2.05(189) \times 10^{-5}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{标度特征剖析}：
\begin{enumerate}
    \item $N_t=12$（对应 $T=204.41\text{ MeV}$，高温相）：手征凝聚为负微小量（$-2.05 \times 10^{-5}$），在误差范围内严格与零兼容，表明对称性完全恢复；
    \item $N_t=14$（对应 $T=175.21\text{ MeV}$）：手征凝聚趋于零（$-1.15 \times 10^{-8}$），处于相变后边缘；
    \item $N_t=18$（对应 $T=136.28\text{ MeV}$，低温相）：凝聚回升至 $(8.30 \pm 0.44) \times 10^{-4}$，明确验证了低温区自发破缺的恢复机制。
\end{enumerate}
...
图 \ref{fig:sucep_comparison} 展示了 No-RM 与 RM-Corrected 两种模式下手征磁化率随温度演化的全景对比曲线，并渲染了 $T \in [155.5, 160.5]\text{ MeV}$ 的交叉过渡带。
图 \ref{fig:sucep_subfigures} 则分别展示了严格对标 \texttt{ana/ttest} 原始配色与样式的单图（红色 No-RM 与靛蓝色标度 RM-Corrected）。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{docs/figures/sucep_comparison.pdf}
    \caption{手征磁化率 No-RM（红色虚线方块）与 RM 修正（靛蓝色实线圆点）全量对比图。阴影区域标示交叉过渡带。峰值清晰定位于 $\beta=4.18, T=157.0\text{ MeV}$，精确对应手征相变赝临界温度 $T_{\text{pc}}$。}
    \label{fig:sucep_comparison}
\end{figure}
...
\begin{enumerate}
    \item \textbf{峰值精准吻合拐点}：比较图 \ref{fig:sucep_comparison} 与图 \ref{fig:conden_re_plot} 可以发现，手征磁化率的极大值峰顶出现在 $T = 157.0\text{ MeV}$（对应 $\beta=4.18$），该温度与重整化手征凝聚 $\Delta_{\ell, s}(T)$ 曲线的一阶导数极大值（拐点）完全重合，确凿印证了 QCD 热相变为热力学光滑交叉转变（Crossover）；
