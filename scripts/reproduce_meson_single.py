#!/usr/bin/env python3
"""
scripts/reproduce_meson_single.py
--------------------------------------------------------------------------------
Single-Source Meson Correlator Pipeline (Polars + NumPy accelerated):
1. Extract single-source correlators via MesonOrchestrator
...
from docs.physics_setup import (
    BETAS, CHANNELS, CHANNEL_CONFIGS, SINGLE_FIT_SLICES,
...
def main() -> None:
    ...
    print("[INFO] Starting Single-Source Meson pipeline (Polars + NumPy accelerated)...")
    ...
    for beta in BETAS:
        ...
        pick_dir = OUTPUT_ROOT / f"pickdata-singlesrc/b4.{beta}"
        pick_dir.mkdir(parents=True, exist_ok=True)

        t0 = time.time()
        for ch in CHANNELS:
            configs = CHANNEL_CONFIGS[ch]
            # 位置参数：传入 True 表示 single-source
            binsize = get_binsize(beta, ch, True)
        ...
        print(f"  - Single-Source Beta 4.{beta}: 6 channels extracted ({elapsed:.2f}s, {n_cfgs} cfgs, {n_bins} bins)")

    print("\n[INFO] Single-Source: Computing effective mass (meff)...")
    meff_tasks = []
    for beta in BETAS:
        out_dir = OUTPUT_ROOT / "ratio_results-singlesrc" / f"b4.{beta}"
        out_dir.mkdir(parents=True, exist_ok=True)
        for ch in CHANNELS:
            dr_path = OUTPUT_ROOT / "pickdata-singlesrc" / f"b4.{beta}" / f"dr_{ch}.csv"
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for f in as_completed(futures):
            f.result()
    print(f"  - Single-Source meff tasks completed in {time.time() - t0_meff:.2f}s")

    print("\n[INFO] Single-Source: Performing Bayesian cosh plateau fits...")
    fit_tasks = []
    for beta in BETAS:
        out_dir = OUTPUT_ROOT / "simulateresult-singlesrc" / f"b4.{beta}"
        out_dir.mkdir(parents=True, exist_ok=True)
        for ch in CHANNELS:
            dr_path = OUTPUT_ROOT / "pickdata-singlesrc" / f"b4.{beta}" / f"dr_{ch}.csv"
            err_path = OUTPUT_ROOT / "pickdata-singlesrc" / f"b4.{beta}" / f"err_{ch}.csv"
            t_start = SINGLE_FIT_SLICES[beta][ch]
            t_end = FIT_SLICE_END
    ...
    summary_path = OUTPUT_ROOT / "simulateresult-singlesrc" / "all_fits_summary.csv"
    summary_df.write_csv(summary_path)
    print(f"  - Single-Source fits saved: {summary_path}")

    total_time = time.time() - t_start_total
    print(f"\n[INFO] Single-Source Meson pipeline completed successfully in {total_time:.2f}s")
